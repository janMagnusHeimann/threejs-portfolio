...
import { useMobileVideoTexture } from '../hooks/useMobileVideoTexture';
...
const DemoComputer = (props) => {
  ...
  const videoPath = props.texture || '/textures/project/project1.mp4';
  const fallbackImage = generateFallbackImage(videoPath);
  
  const { texture: videoTexture, shouldUseVideo, hasError, deviceInfo } = useMobileVideoTexture(videoPath, fallbackImage);
  
  // Fallback to static image if video fails or is not supported
  const fallbackTexture = useTexture(fallbackImage);
  
  // Choose appropriate texture
  const txt = shouldUseVideo && !hasError ? videoTexture : fallbackTexture;
      ...
      <group name="Scene">
        <mesh
          ...
          <meshBasicMaterial 
            map={txt} 
            toneMapped={false}
            transparent={!shouldUseVideo}
            opacity={shouldUseVideo ? 1 : 0.8}
          />
        </mesh>
